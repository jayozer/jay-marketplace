...
- **Launch is a handover, never an action.** Claude Code exposes no goal tool to the model; `/goal <condition>` is a user command. In explicit run mode, return the exact `/goal` block for the user to run; for a headless run, write the block to a file and run `claude -p "$(cat goal.md)"`. Never inline the block in double quotes, because the shell would execute its backticked commands and expand `$` before Claude starts. Never report a goal as created, active, or launched.
