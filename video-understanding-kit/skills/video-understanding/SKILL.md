...
```bash
python3 -m pip install -r "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/video-understanding/requirements.txt"
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/video-understanding/analyze_video_gemini.py" "<video_path>" \
  --question "<question>" \
  --mode standard \
